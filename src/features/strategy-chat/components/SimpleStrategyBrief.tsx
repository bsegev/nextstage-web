...
interface SimpleStrategyBriefProps {
...
// Generate brief locally without API calls for now
const generateLocalBrief = (responses: UserResponse[]): Brief => {
...
export const SimpleStrategyBrief = ({ responses, submissionId, onRefine, onStartOver }: SimpleStrategyBriefProps) => {
  ...
  const loadBrief = async () => {
    try {
      // Try to generate the brief via API first
      console.log('Attempting to generate brief via API...');
      ...
      if (response.ok) {
        ...
        setBrief(brief);
        console.log('Brief generated successfully via API');
      } else {
        throw new Error('API call failed');
      }
    } catch (error) {
      console.error('Error generating brief via API, using local generation:', error);
      // Fallback to local brief generation
      const localBrief = generateLocalBrief(responses);
      setBrief(localBrief);
      console.log('Using locally generated brief');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBookCall = async () => {
    try {
      console.log('Mock: Calendly click tracked');
      window.open("https://cal.com/bensegev/30min", "_blank");
    } catch (error) {
      console.error('Error tracking calendly click:', error);
      window.open("https://cal.com/bensegev/30min", "_blank");
    }
  };
  ...
  const handlePrintDownload = async () => {
    try {
      console.log('Mock: PDF download tracked');
      window.print();
    } catch (error) {
